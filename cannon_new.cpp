...
vector<int> reformat()
{
	...
	vector<int> v(2*soldiers_number,0);
	for(int i =0; i<soldiers_number;i++)
	{
		v[i]=soldiers1[i];
		v[i+soldiers_number]=(soldiers2[i]==-1)?soldiers2[i]:-soldiers2[i];
	}
	sort(v.begin(),v.end());
	return v;
}
...
float evaluation_function(int show)
{
	...
	//0.5*(soldiers_under_fire2 - soldiers_under_fire0) + 20*(townhalls_under_fire2-townhalls_under_fire0) + 10*(number_of_cannons0-number_of_cannons2) + 3*(number_of_cannons_horiz0-number_of_cannons_horiz2);
	if (show==1)
...
float minimax(int player_id, int depth , float alpha, float beta,float &possible_moves2_time, float &evaluation_function_time , float &cannon_related_time)
{
	...
	vector<int> ref_vec = reformat();
	if(table[depth][ref_vec]!=0)
		return table[depth][ref_vec];
...
// Check initial player numbers
// Weights on soldiers under fire
